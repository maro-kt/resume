...
VR コンテンツを簡単に編集できる SaaS アプリケーションが稼働しているパブリッククラウドのインフラ設計、監視設計、セキュリティ、ビッグデータ基盤の整備を中心に行っている。また、日々の開発環境におけるフローや課題管理などの組織レベルでの運用改善も行っている。
...
* ビッグデータ： BigQuery
* モニタリング： DataDog
* セキュリティ： AWS Security Hub、GuardDuty、WAF、Cloud Asset Inventory、Security Command Center
* プロジェクト管理： Jira
* コード管理：GitHub
* IaC ： Terraform Cloud
...
1. AWS、GCP におけるセキュリティ基盤の整備。認証周り、インシデント管理、監査ログなどのセキュリティ運用で必要な基盤を整備。これにより、以前は十分に管理が行き届いてなかったセキュリティ周りのイベント管理を可視化できるようになった。
1. 社内データ分析基盤の強化。AWS Athena から BigQuery へのマイグレーション、および Salesforce とのデータ連携。Tableau を用いたデータ分析ツールの整備などを主導。これにより、以前よりも高速なデータ分析基盤が構築できた。
...
### 株式会社ダイハツビジネスサポートセンター | システム管理者（2013/07 - 2014/06)
